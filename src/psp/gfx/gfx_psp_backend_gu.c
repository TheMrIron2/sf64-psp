...
void PspGfxBackend_DrawSolidRect(float ulx, float uly, float lrx, float lry, u32 color, int blend,
    ...
    PspGfxGuColorVertex vertices[6] __attribute__((aligned(16)));

    if (blend || (lrx <= ulx) || (lry <= uly) || !psp_gfx_gu_select_viewport(viewport)) {
        return;
    }

    vertices[0] = (PspGfxGuColorVertex) { color, (ulx / 160.0f) - 1.0f, 1.0f - (uly / 120.0f), 0.0f };
    vertices[1] = (PspGfxGuColorVertex) { color, (lrx / 160.0f) - 1.0f, 1.0f - (uly / 120.0f), 0.0f };
    vertices[2] = (PspGfxGuColorVertex) { color, (lrx / 160.0f) - 1.0f, 1.0f - (lry / 120.0f), 0.0f };
    vertices[3] = vertices[0];
    vertices[4] = vertices[2];
    vertices[5] = (PspGfxGuColorVertex) { color, (ulx / 160.0f) - 1.0f, 1.0f - (lry / 120.0f), 0.0f };
