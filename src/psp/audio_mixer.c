/*
 * Portable scalar N64 audio ABI mixer adapted from HarbourMasters/Starship
 * commit 6202c44356fee70dd23e80a16933b211863d3e2d (CC0-1.0).
 */
#include "sys.h"
...
void aLoadADPCMImpl(int num_entries_times_16, const int16_t* book_source_addr) {
    s32 i;
    s32 count = num_entries_times_16 / sizeof(int16_t);
    int16_t* out = &rspa.adpcm_table[0][0][0];

    for (i = 0; i < count; i++) {
        out[i] = __builtin_bswap16(book_source_addr[i]);
    }
}
...
void aSetLoopImpl(ADPCM_STATE* adpcm_loop_state) {
    ...
    for (i = 0; i < 16; i++) {
        rspa.adpcm_loop_state[i] = __builtin_bswap16((*adpcm_loop_state)[i]);
    }
}
...
void aEnvMixerImpl(uint16_t in_addr, uint16_t n_samples, bool swap_reverb, bool x0, bool x1, bool x2, bool x3,
    ...
    if (!sLoggedEnvelopeSignalContext && (envelopeInputPeak != 0)) {
        ...
        PspPlatform_LogValue("audio signal envelope flags", ((u32) swap_reverb << 4) | ((u32) x0 << 3) |
    ...
    if (num_channels == 6) {
        ...
        for (int i = 0; i < n / 8; i++) {
            for (int k = 0; k < 8; k++) {
                ...
                samples[0] ^= x2 ? -1 : 0;
                samples[1] ^= x3 ? -1 : 0;
                wet_samples[0] = samples[0] * vol_wet >> 16;
                wet_samples[1] = samples[1] * vol_wet >> 16;
                wet_samples[0] ^= x0 ? -4 : 0;
                wet_samples[1] ^= x1 ? -2 : 0;
                ...
                for (int j = 0; j < 2; j++) {
                    *dry[j] = clamp16(*dry[j] + samples[j]);
                    ...
                    // Apply reverb
                    *wet[j] = clamp16(*wet[j] + wet_samples[swapped[j]]);
                    wet[j]++;
                }
            }
