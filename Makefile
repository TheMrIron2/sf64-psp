...
psp-profile-gprof:
	$(MAKE) PROFILE_PSP=1 SF64_PSP_PROFILE_PHASES=0 BUILD_DIR=build/psp-profile-gprof PSP_FPS_OVERLAY=$(PSP_FPS_OVERLAY) PSP_LOG=$(PSP_LOG) PSP_TRACE=$(PSP_TRACE) PSP_RENDERER_DIAGNOSTICS=$(PSP_RENDERER_DIAGNOSTICS) psp

psp-profile-phases:
	$(MAKE) PROFILE_PSP=0 SF64_PSP_PROFILE_PHASES=1 BUILD_DIR=build/psp-profile-phases PSP_FPS_OVERLAY=$(PSP_FPS_OVERLAY) PSP_LOG=$(PSP_LOG) PSP_TRACE=$(PSP_TRACE) PSP_RENDERER_DIAGNOSTICS=$(PSP_RENDERER_DIAGNOSTICS) psp

psp-profile-combined:
	$(MAKE) PROFILE_PSP=1 SF64_PSP_PROFILE_PHASES=1 BUILD_DIR=build/psp-profile-combined PSP_FPS_OVERLAY=$(PSP_FPS_OVERLAY) PSP_LOG=$(PSP_LOG) PSP_TRACE=$(PSP_TRACE) PSP_RENDERER_DIAGNOSTICS=$(PSP_RENDERER_DIAGNOSTICS) psp
...
$(PROFILE_METADATA): $(PSP_EBOOT) $(PSP_ELF) $(PSP_MAP) Makefile
	...
	$(V){ \
		...
		printf 'build_command=make %s BUILD_DIR=%s PROFILE_PSP=%s SF64_PSP_PROFILE_PHASES=%s SF64_PSP_PROFILE_CAPTURE_FRAMES=%s SF64_PSP_PROFILE_FRAME_TRACE=%s SF64_PSP_PROFILE_FRAME_TRACE_FRAMES=%s SF64_PSP_PROFILE_COMPONENTS=%s SF64_PSP_PROFILE_TRIVIAL_REJECTS=%s USE_LOCAL_PSPGL=%s SF64_PSP_BATCH_STATE_CACHE=%s PSP_FPS_OVERLAY=%s PSP_LOG=%s PSP_TRACE=%s PSP_RENDERER_DIAGNOSTICS=%s psp\n' '$(MAKECMDGOALS)' '$(BUILD_DIR)' '$(PROFILE_PSP)' '$(SF64_PSP_PROFILE_PHASES)' '$(SF64_PSP_PROFILE_CAPTURE_FRAMES)' '$(SF64_PSP_PROFILE_FRAME_TRACE)' '$(SF64_PSP_PROFILE_FRAME_TRACE_FRAMES)' '$(SF64_PSP_PROFILE_COMPONENTS)' '$(SF64_PSP_PROFILE_TRIVIAL_REJECTS)' '$(USE_LOCAL_PSPGL)' '$(SF64_PSP_BATCH_STATE_CACHE)' '$(PSP_FPS_OVERLAY)' '$(PSP_LOG)' '$(PSP_TRACE)' '$(PSP_RENDERER_DIAGNOSTICS)'; \
	} > $@
