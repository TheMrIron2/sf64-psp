...
SF64_PSP_PROFILE_TRIVIAL_REJECTS ?= 0
SF64_PSP_PROFILE_VERTEX_REUSE ?= 0
...
SF64_PSP_EARLY_TRIVIAL_REJECT ?= 1
...
# Diagnostic-only A/B experiments for the gameplay cloud regression
# SF64_PSP_CLOUD_IA16_SWAP_EXPERIMENT: 1 = swap decoded intensity/alpha byte order for aPlCloudsTex only
SF64_PSP_CLOUD_IA16_SWAP_EXPERIMENT ?= 0
# SF64_PSP_CLOUD_ALPHA_FLOOR_EXPERIMENT: N>0 = zero decoded alpha below N (e.g. 8/16/32) for aPlCloudsTex only
SF64_PSP_CLOUD_ALPHA_FLOOR_EXPERIMENT ?= 0
# SF64_PSP_CLOUD_POINT_FILTER_EXPERIMENT: 1 = force point (nearest) filtering for aPlCloudsTex only
SF64_PSP_CLOUD_POINT_FILTER_EXPERIMENT ?= 0
PSP_AUDIO_SYNTH ?= 0
PSP_AUDIO_OUTPUT ?= 0
PSP_FPS_OVERLAY ?= 1
N64PSP_USE_VFPU ?= 1
...
USE_N64PSP_VERTEX_CHAIN2 ?= $(USE_N64PSP_BATCH_TRANSFORM)
N64PSP_VERTEX_CHAIN2_VALIDATE ?= $(PSP_VALIDATE_N64PSP_BATCH_TRANSFORM)
N64PSP_VERTEX_BATCH_DIAGNOSTICS ?= 0
...
ifeq ($(SF64_PSP_PSPGL_PROFILE),1)
ifneq ($(USE_LOCAL_PSPGL),1)
$(error SF64_PSP_PSPGL_PROFILE=1 requires USE_LOCAL_PSPGL=1 so the instrumented local PSPGL archive is built and linked.)
endif
endif
ifeq ($(SF64_PSP_PROFILE_FRAME_TRACE),1)
ifneq ($(SF64_PSP_PROFILE_PHASES),1)
$(error SF64_PSP_PROFILE_FRAME_TRACE=1 requires SF64_PSP_PROFILE_PHASES=1.)
endif
ifeq ($(SF64_PSP_PROFILE_FRAME_TRACE_FRAMES),0)
$(error SF64_PSP_PROFILE_FRAME_TRACE_FRAMES must be at least 1.)
endif
ifneq ($(shell test "$(SF64_PSP_PROFILE_FRAME_TRACE_FRAMES)" -le 3600 2>/dev/null && echo ok),ok)
$(error SF64_PSP_PROFILE_FRAME_TRACE_FRAMES=$(SF64_PSP_PROFILE_FRAME_TRACE_FRAMES) is unreasonable; use 1..3600.)
endif
endif
ifeq ($(SF64_PSP_PROFILE_COMPONENTS),1)
ifneq ($(SF64_PSP_PROFILE_PHASES),1)
$(error SF64_PSP_PROFILE_COMPONENTS=1 requires SF64_PSP_PROFILE_PHASES=1.)
endif
endif
ifeq ($(SF64_PSP_PROFILE_TRIVIAL_REJECTS),1)
ifneq ($(SF64_PSP_PROFILE_PHASES),1)
$(error SF64_PSP_PROFILE_TRIVIAL_REJECTS=1 requires SF64_PSP_PROFILE_PHASES=1.)
endif
endif
ifneq ($(filter 0 1,$(SF64_PSP_PROFILE_VERTEX_REUSE)),$(SF64_PSP_PROFILE_VERTEX_REUSE))
$(error SF64_PSP_PROFILE_VERTEX_REUSE must be 0 or 1, got '$(SF64_PSP_PROFILE_VERTEX_REUSE)'.)
endif
ifeq ($(SF64_PSP_PROFILE_VERTEX_REUSE),1)
ifneq ($(SF64_PSP_PROFILE_PHASES),1)
$(error SF64_PSP_PROFILE_VERTEX_REUSE=1 requires SF64_PSP_PROFILE_PHASES=1.)
endif
endif
ifeq ($(SF64_PSP_TRI2_PAIR_VALIDATE),1)
ifneq ($(SF64_PSP_TRI2_PAIR_FASTPATH),1)
$(error SF64_PSP_TRI2_PAIR_VALIDATE=1 requires SF64_PSP_TRI2_PAIR_FASTPATH=1.)
endif
ifneq ($(SF64_PSP_PROFILE_PHASES),1)
$(error SF64_PSP_TRI2_PAIR_VALIDATE=1 requires SF64_PSP_PROFILE_PHASES=1.)
endif
endif
ifneq ($(filter 0 1,$(SF64_PSP_EARLY_TRIVIAL_REJECT)),$(SF64_PSP_EARLY_TRIVIAL_REJECT))
$(error SF64_PSP_EARLY_TRIVIAL_REJECT must be 0 or 1, got '$(SF64_PSP_EARLY_TRIVIAL_REJECT)'.)
endif
ifneq ($(SF64_PSP_PSPGL_DLIST_SIZE_WORDS),512)
ifneq ($(USE_LOCAL_PSPGL),1)
$(error SF64_PSP_PSPGL_DLIST_SIZE_WORDS=$(SF64_PSP_PSPGL_DLIST_SIZE_WORDS) requires USE_LOCAL_PSPGL=1 so the local PSPGL archive is built with the requested command-list capacity.)
endif
endif
ifeq ($(PROFILE_PSP),1)
...
CFLAGS += -DUSE_N64PSP_SINCOS=$(if $(filter 1,$(USE_N64PSP_SINCOS)),1,0)
...
CFLAGS += -DSF64_PSP_PROFILE_FRAME_TRACE=$(if $(filter 1,$(SF64_PSP_PROFILE_FRAME_TRACE)),1,0)
CFLAGS += -DSF64_PSP_PROFILE_FRAME_TRACE_FRAMES=$(SF64_PSP_PROFILE_FRAME_TRACE_FRAMES)
CFLAGS += -DSF64_PSP_PROFILE_COMPONENTS=$(if $(filter 1,$(SF64_PSP_PROFILE_COMPONENTS)),1,0)
CFLAGS += -DSF64_PSP_PROFILE_TRIVIAL_REJECTS=$(if $(filter 1,$(SF64_PSP_PROFILE_TRIVIAL_REJECTS)),1,0)
CFLAGS += -DSF64_PSP_PROFILE_VERTEX_REUSE=$(if $(filter 1,$(SF64_PSP_PROFILE_VERTEX_REUSE)),1,0)
CFLAGS += -DSF64_PSP_PSPGL_PROFILE=$(if $(filter 1,$(SF64_PSP_PSPGL_PROFILE)),1,0)
CFLAGS += -DSF64_PSP_PSPGL_DLIST_SIZE_WORDS=$(SF64_PSP_PSPGL_DLIST_SIZE_WORDS)
...
CFLAGS += -DSF64_PSP_TRI2_PAIR_FASTPATH=$(if $(filter 1,$(SF64_PSP_TRI2_PAIR_FASTPATH)),1,0)
CFLAGS += -DSF64_PSP_TRI2_PAIR_VALIDATE=$(if $(filter 1,$(SF64_PSP_TRI2_PAIR_VALIDATE)),1,0)
CFLAGS += -DSF64_PSP_BATCH_STATE_CACHE=$(if $(filter 1,$(SF64_PSP_BATCH_STATE_CACHE)),1,0)
CFLAGS += -DSF64_PSP_EARLY_TRIVIAL_REJECT=$(if $(filter 1,$(SF64_PSP_EARLY_TRIVIAL_REJECT)),1,0)
CFLAGS += -DSF64_PSP_TEXTURE_WRAP_CACHE=$(if $(filter 1,$(SF64_PSP_TEXTURE_WRAP_CACHE)),1,0)
...
CFLAGS += -DUSE_N64PSP_VERTEX_CHAIN2=$(if $(filter 1,$(USE_N64PSP_VERTEX_CHAIN2)),1,0)
CFLAGS += -DN64PSP_VERTEX_CHAIN2_VALIDATE=$(if $(filter 1,$(N64PSP_VERTEX_CHAIN2_VALIDATE)),1,0)
CFLAGS += -DN64PSP_VERTEX_BATCH_DIAGNOSTICS=$(if $(filter 1,$(N64PSP_VERTEX_BATCH_DIAGNOSTICS)),1,0)
...
CFLAGS += -DSF64_PSP_CLOUD_IA16_SWAP_EXPERIMENT=$(SF64_PSP_CLOUD_IA16_SWAP_EXPERIMENT)
CFLAGS += -DSF64_PSP_CLOUD_ALPHA_FLOOR_EXPERIMENT=$(SF64_PSP_CLOUD_ALPHA_FLOOR_EXPERIMENT)
CFLAGS += -DSF64_PSP_CLOUD_POINT_FILTER_EXPERIMENT=$(SF64_PSP_CLOUD_POINT_FILTER_EXPERIMENT)
...
ifeq ($(USE_N64PSP_VERTEX_CHAIN2),1)
ifneq ($(USE_N64PSP_MATH),1)
$(error USE_N64PSP_VERTEX_CHAIN2=1 requires USE_N64PSP_MATH=1)
endif
endif
ifeq ($(N64PSP_VERTEX_CHAIN2_VALIDATE),1)
ifneq ($(USE_N64PSP_VERTEX_CHAIN2),1)
$(error N64PSP_VERTEX_CHAIN2_VALIDATE=1 requires USE_N64PSP_VERTEX_CHAIN2=1)
endif
endif
ifeq ($(USE_N64PSP_SINCOS),1)
ifneq ($(USE_N64PSP_MATH),1)
$(error USE_N64PSP_SINCOS=1 requires USE_N64PSP_MATH=1)
endif
endif
...
psp-profile-gprof psp-profile-phases psp-profile-combined psp-profile-builds psp-profile-artifacts: N64PSP_VERTEX_CHAIN2_VALIDATE=0
psp-profile-gprof psp-profile-phases psp-profile-combined psp-profile-builds psp-profile-artifacts: N64PSP_VERTEX_BATCH_DIAGNOSTICS=0
psp-profile-gprof psp-profile-phases psp-profile-combined psp-profile-builds psp-profile-artifacts: N64PSP_QUEUE_TRACE=0

psp-profile-gprof:
	$(MAKE) PROFILE_PSP=1 SF64_PSP_PROFILE_PHASES=0 BUILD_DIR=build/psp-profile-gprof PSP_FPS_OVERLAY=$(PSP_FPS_OVERLAY) PSP_LOG=$(PSP_LOG) PSP_TRACE=$(PSP_TRACE) PSP_RENDERER_DIAGNOSTICS=$(PSP_RENDERER_DIAGNOSTICS) PSP_VALIDATE_N64PSP_MATH=$(PSP_VALIDATE_N64PSP_MATH) USE_N64PSP_SINCOS=$(USE_N64PSP_SINCOS) USE_N64PSP_VERTEX_CHAIN2=$(USE_N64PSP_VERTEX_CHAIN2) N64PSP_VERTEX_CHAIN2_VALIDATE=$(N64PSP_VERTEX_CHAIN2_VALIDATE) N64PSP_VERTEX_BATCH_DIAGNOSTICS=$(N64PSP_VERTEX_BATCH_DIAGNOSTICS) N64PSP_QUEUE_TRACE=$(N64PSP_QUEUE_TRACE) psp
...
$(PROFILE_METADATA): $(PSP_EBOOT) $(PSP_ELF) $(PSP_MAP) Makefile
	...
	$(V){ \
		...
		printf 'pspgl_profile=%s\n' '$(SF64_PSP_PSPGL_PROFILE)'; \
		printf 'pspgl_dlist_size_words=%s\n' '$(SF64_PSP_PSPGL_DLIST_SIZE_WORDS)'; \
		...
		printf 'SF64_PSP_PROFILE_FRAME_TRACE=%s\n' '$(SF64_PSP_PROFILE_FRAME_TRACE)'; \
		printf 'SF64_PSP_PROFILE_FRAME_TRACE_FRAMES=%s\n' '$(SF64_PSP_PROFILE_FRAME_TRACE_FRAMES)'; \
		printf 'SF64_PSP_PROFILE_COMPONENTS=%s\n' '$(SF64_PSP_PROFILE_COMPONENTS)'; \
		printf 'SF64_PSP_PROFILE_TRIVIAL_REJECTS=%s\n' '$(SF64_PSP_PROFILE_TRIVIAL_REJECTS)'; \
		printf 'SF64_PSP_PROFILE_VERTEX_REUSE=%s\n' '$(SF64_PSP_PROFILE_VERTEX_REUSE)'; \
		printf 'SF64_PSP_PSPGL_PROFILE=%s\n' '$(SF64_PSP_PSPGL_PROFILE)'; \
		printf 'SF64_PSP_PSPGL_DLIST_SIZE_WORDS=%s\n' '$(SF64_PSP_PSPGL_DLIST_SIZE_WORDS)'; \
		...
		printf 'SF64_PSP_TRI2_PAIR_FASTPATH=%s\n' '$(SF64_PSP_TRI2_PAIR_FASTPATH)'; \
		printf 'SF64_PSP_TRI2_PAIR_VALIDATE=%s\n' '$(SF64_PSP_TRI2_PAIR_VALIDATE)'; \
		printf 'SF64_PSP_BATCH_STATE_CACHE=%s\n' '$(SF64_PSP_BATCH_STATE_CACHE)'; \
		printf 'SF64_PSP_EARLY_TRIVIAL_REJECT=%s\n' '$(SF64_PSP_EARLY_TRIVIAL_REJECT)'; \
		...
		printf 'USE_N64PSP_SINCOS=%s\n' '$(USE_N64PSP_SINCOS)'; \
		printf 'PSP_VALIDATE_N64PSP_BATCH_TRANSFORM=%s\n' '$(PSP_VALIDATE_N64PSP_BATCH_TRANSFORM)'; \
		printf 'USE_N64PSP_VERTEX_CHAIN2=%s\n' '$(USE_N64PSP_VERTEX_CHAIN2)'; \
		printf 'N64PSP_VERTEX_CHAIN2_VALIDATE=%s\n' '$(N64PSP_VERTEX_CHAIN2_VALIDATE)'; \
		printf 'N64PSP_VERTEX_BATCH_DIAGNOSTICS=%s\n' '$(N64PSP_VERTEX_BATCH_DIAGNOSTICS)'; \
		...
		printf 'build_command=make %s BUILD_DIR=%s PROFILE_PSP=%s SF64_PSP_PROFILE_PHASES=%s SF64_PSP_PROFILE_CAPTURE_FRAMES=%s SF64_PSP_PROFILE_FRAME_TRACE=%s SF64_PSP_PROFILE_FRAME_TRACE_FRAMES=%s SF64_PSP_PROFILE_COMPONENTS=%s SF64_PSP_PROFILE_TRIVIAL_REJECTS=%s SF64_PSP_PROFILE_VERTEX_REUSE=%s SF64_PSP_PSPGL_PROFILE=%s SF64_PSP_PSPGL_DLIST_SIZE_WORDS=%s SF64_PSP_PSPGL_VBO_STREAM=%s USE_LOCAL_PSPGL=%s SF64_PSP_DIRECT_TRI_FASTPATH=%s SF64_PSP_TRI2_PAIR_FASTPATH=%s SF64_PSP_TRI2_PAIR_VALIDATE=%s SF64_PSP_BATCH_STATE_CACHE=%s SF64_PSP_EARLY_TRIVIAL_REJECT=%s SF64_PSP_TEXTURE_WRAP_CACHE=%s PSP_FPS_OVERLAY=%s PSP_LOG=%s PSP_TRACE=%s PSP_RENDERER_DIAGNOSTICS=%s PSP_VALIDATE_N64PSP_MATH=%s USE_N64PSP_SINCOS=%s PSP_VALIDATE_N64PSP_BATCH_TRANSFORM=%s USE_N64PSP_VERTEX_CHAIN2=%s N64PSP_VERTEX_CHAIN2_VALIDATE=%s N64PSP_VERTEX_BATCH_DIAGNOSTICS=%s N64PSP_QUEUE_TRACE=%s psp\n' '$(MAKECMDGOALS)' '$(BUILD_DIR)' '$(PROFILE_PSP)' '$(SF64_PSP_PROFILE_PHASES)' '$(SF64_PSP_PROFILE_CAPTURE_FRAMES)' '$(SF64_PSP_PROFILE_FRAME_TRACE)' '$(SF64_PSP_PROFILE_FRAME_TRACE_FRAMES)' '$(SF64_PSP_PROFILE_COMPONENTS)' '$(SF64_PSP_PROFILE_TRIVIAL_REJECTS)' '$(SF64_PSP_PROFILE_VERTEX_REUSE)' '$(SF64_PSP_PSPGL_PROFILE)' '$(SF64_PSP_PSPGL_DLIST_SIZE_WORDS)' '$(SF64_PSP_PSPGL_VBO_STREAM)' '$(USE_LOCAL_PSPGL)' '$(SF64_PSP_DIRECT_TRI_FASTPATH)' '$(SF64_PSP_TRI2_PAIR_FASTPATH)' '$(SF64_PSP_TRI2_PAIR_VALIDATE)' '$(SF64_PSP_BATCH_STATE_CACHE)' '$(SF64_PSP_EARLY_TRIVIAL_REJECT)' '$(SF64_PSP_TEXTURE_WRAP_CACHE)' '$(PSP_FPS_OVERLAY)' '$(PSP_LOG)' '$(PSP_TRACE)' '$(PSP_RENDERER_DIAGNOSTICS)' '$(PSP_VALIDATE_N64PSP_MATH)' '$(USE_N64PSP_SINCOS)' '$(PSP_VALIDATE_N64PSP_BATCH_TRANSFORM)' '$(USE_N64PSP_VERTEX_CHAIN2)' '$(N64PSP_VERTEX_CHAIN2_VALIDATE)' '$(N64PSP_VERTEX_BATCH_DIAGNOSTICS)' '$(N64PSP_QUEUE_TRACE)'; \
	} > $@
...
$(LOCAL_PSPGL_LIB): FORCE
	env -u MAKEFLAGS $(MAKE) -C $(PSPGL_DIR) .deps libGL.a \
		...
		AR=psp-ar \
		RANLIB=psp-ranlib \
		PSPGL_PROFILE=$(SF64_PSP_PSPGL_PROFILE) \
		PSPGL_DLIST_SIZE_WORDS=$(SF64_PSP_PSPGL_DLIST_SIZE_WORDS)
