...
$(PROFILE_METADATA): $(PSP_EBOOT) $(PSP_ELF) $(PSP_MAP) Makefile
	...
	$(V){ \
		...
		printf 'build_command=make %s BUILD_DIR=%s PROFILE_PSP=%s SF64_PSP_PROFILE_PHASES=%s SF64_PSP_PSPGL_VBO_STREAM=%s PSP_FPS_OVERLAY=%s PSP_LOG=%s PSP_TRACE=%s PSP_RENDERER_DIAGNOSTICS=%s PSP_VALIDATE_N64PSP_MATH=%s PSP_VALIDATE_N64PSP_BATCH_TRANSFORM=%s N64PSP_QUEUE_TRACE=%s psp\n' '$(MAKECMDGOALS)' '$(BUILD_DIR)' '$(PROFILE_PSP)' '$(SF64_PSP_PROFILE_PHASES)' '$(SF64_PSP_PSPGL_VBO_STREAM)' '$(PSP_FPS_OVERLAY)' '$(PSP_LOG)' '$(PSP_TRACE)' '$(PSP_RENDERER_DIAGNOSTICS)' '$(PSP_VALIDATE_N64PSP_MATH)' '$(PSP_VALIDATE_N64PSP_BATCH_TRANSFORM)' '$(N64PSP_QUEUE_TRACE)'; \
	} > $@
