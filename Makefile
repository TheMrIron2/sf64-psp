...
PSP_AUDIO_PROFILE ?= 0
PSP_AUDIO_VME ?= 0
PSP_AUDIO_VME_VALIDATE ?= 0
PSP_AUDIO_VME_BENCH ?= 0
PSP_FPS_OVERLAY ?= 1
COLOR ?= 1
VERBOSE ?= 0
...
ifneq ($(PSP_AUDIO_VME_VALIDATE),0)
ifneq ($(PSP_AUDIO_VME_VALIDATE),1)
$(error PSP_AUDIO_VME_VALIDATE must be 0 or 1)
endif
endif
ifeq ($(PSP_AUDIO_VME_VALIDATE),1)
ifneq ($(PSP_AUDIO_VME),1)
$(error PSP_AUDIO_VME_VALIDATE=1 requires PSP_AUDIO_VME=1)
endif
endif
ifneq ($(PSP_AUDIO_VME_BENCH),0)
ifneq ($(PSP_AUDIO_VME_BENCH),1)
$(error PSP_AUDIO_VME_BENCH must be 0 or 1)
endif
endif
ifeq ($(PSP_AUDIO_VME_BENCH),1)
ifneq ($(PSP_AUDIO_VME_VALIDATE),1)
$(error PSP_AUDIO_VME_BENCH=1 requires PSP_AUDIO_VME_VALIDATE=1)
endif
ifneq ($(PSP_AUDIO_PROFILE),1)
$(error PSP_AUDIO_VME_BENCH=1 requires PSP_AUDIO_PROFILE=1)
endif
endif
...
psp-profile-hw-counters:
	$(MAKE) PROFILE_PSP=0 PROFILE_PHASES=0 PROFILE_COMPONENTS=0 PROFILE_HW_COUNTERS=1 PROFILE_HW_COUNTER_SCOPES=$(PROFILE_HW_COUNTER_SCOPES) PROFILE_HW_COUNTER_FRAMES=$(PROFILE_HW_COUNTER_FRAMES) PROFILE_HW_COUNTER_WARMUP_FRAMES=$(PROFILE_HW_COUNTER_WARMUP_FRAMES) BUILD_DIR=build/psp-profile-hw-counters PSP_FPS_OVERLAY=$(PSP_FPS_OVERLAY) PSP_LOG=$(PSP_LOG) PSP_TRACE=$(PSP_TRACE) PSP_RENDERER_DIAGNOSTICS=$(PSP_RENDERER_DIAGNOSTICS) PSP_AUDIO=$(PSP_AUDIO) PSPGL_SWAP_INTERVAL=$(PSPGL_SWAP_INTERVAL) PSP_MAP_SKIP_COMPLETED_PATHS=$(PSP_MAP_SKIP_COMPLETED_PATHS) PSP_MAP_SKIP_HISTORY_HUD=$(PSP_MAP_SKIP_HISTORY_HUD) PSP_MAP_SKIP_PLANETS=$(PSP_MAP_SKIP_PLANETS) USE_LOCAL_PSPGL=$(USE_LOCAL_PSPGL) psp
