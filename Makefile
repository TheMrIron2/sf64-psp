...
#   make PSP_AUDIO=1 PSP_VME=1 psp # VME validation build
...
ifneq ($(PSP_VME),0)
ifneq ($(PSP_VME),1)
$(error PSP_VME must be 0 or 1)
endif
endif
ifeq ($(PSP_VME),1)
ifneq ($(PSP_AUDIO),1)
$(error PSP_VME=1 requires PSP_AUDIO=1)
endif
...
CFLAGS += -DPROFILE_POOL_LOOKUP=$(if $(filter 1,$(PROFILE_POOL_LOOKUP)),1,0)
...
$(PROFILE_METADATA): $(PSP_EBOOT) $(PSP_ELF) $(PSP_MAP) Makefile
	...
	$(V){ \
		...
		printf 'build_command=make %s PSP_GFX_BACKEND=%s BUILD_DIR=%s PROFILE_PSP=%s PROFILE_PHASES=%s PROFILE_CAPTURE_FRAMES=%s PROFILE_FRAME_TRACE=%s PROFILE_FRAME_TRACE_FRAMES=%s PROFILE_COMPONENTS=%s PROFILE_TRIVIAL_REJECTS=%s PROFILE_HW_COUNTERS=$(PROFILE_HW_COUNTERS) PROFILE_HW_COUNTER_SCOPES=$(PROFILE_HW_COUNTER_SCOPES) PROFILE_POOL_LOOKUP=$(PROFILE_POOL_LOOKUP) USE_LOCAL_PSPGL=%s PSPGL_SWAP_INTERVAL=%s PSP_AUDIO=%s PSP_VME=%s PSP_FPS_OVERLAY=%s PSP_LOG=%s PSP_TRACE=%s PSP_RENDERER_DIAGNOSTICS=%s psp\n' '$(MAKECMDGOALS)' '$(PSP_GFX_BACKEND)' '$(BUILD_DIR)' '$(PROFILE_PSP)' '$(PROFILE_PHASES)' '$(PROFILE_CAPTURE_FRAMES)' '$(PROFILE_FRAME_TRACE)' '$(PROFILE_FRAME_TRACE_FRAMES)' '$(PROFILE_COMPONENTS)' '$(PROFILE_TRIVIAL_REJECTS)' '$(USE_LOCAL_PSPGL)' '$(PSPGL_SWAP_INTERVAL)' '$(PSP_AUDIO)' '$(PSP_VME)' '$(PSP_FPS_OVERLAY)' '$(PSP_LOG)' '$(PSP_TRACE)' '$(PSP_RENDERER_DIAGNOSTICS)'; \
	} > $@
