...
SF64_PSP_PSPGL_DIRTY_TEXTURE_FLUSH ?= 0
...
ifeq ($(SF64_PSP_PSPGL_DIRTY_TEXTURE_FLUSH),1)
ifneq ($(USE_LOCAL_PSPGL),1)
$(error SF64_PSP_PSPGL_DIRTY_TEXTURE_FLUSH=1 requires USE_LOCAL_PSPGL=1 so the experimental local PSPGL archive is built and linked.)
endif
endif
...
CFLAGS += -DSF64_PSP_PSPGL_DIRTY_TEXTURE_FLUSH=$(if $(filter 1,$(SF64_PSP_PSPGL_DIRTY_TEXTURE_FLUSH)),1,0)
...
$(PROFILE_METADATA): $(PSP_EBOOT) $(PSP_ELF) $(PSP_MAP) Makefile
	...
	$(V){ \
		...
		printf 'pspgl_dirty_texture_flush=%s\n' '$(SF64_PSP_PSPGL_DIRTY_TEXTURE_FLUSH)'; \
		...
		printf 'SF64_PSP_PSPGL_DIRTY_TEXTURE_FLUSH=%s\n' '$(SF64_PSP_PSPGL_DIRTY_TEXTURE_FLUSH)'; \
...
$(LOCAL_PSPGL_LIB): FORCE
	env -u MAKEFLAGS $(MAKE) -C $(PSPGL_DIR) .deps libGL.a \
		...
		PSPGL_DIRTY_TEXTURE_FLUSH=$(SF64_PSP_PSPGL_DIRTY_TEXTURE_FLUSH)
