...
# Open batch pool, one open batch per texture material
PSP_BATCH_POOL ?= 0
PSP_BATCH_POOL_SLOTS ?= 48
PSP_BATCH_POOL_VERTICES ?= 192
# Draw-merge potential analysis, diagnostic only, reports through the log
PSP_MERGE_ANALYSIS ?= 0
...
ifeq ($(PSP_MERGE_ANALYSIS),1)
CFLAGS += -DPSP_MERGE_ANALYSIS=1
endif
ifeq ($(PSP_BATCH_POOL),1)
CFLAGS += -DPSP_BATCH_POOL=1 -DPSP_BATCH_POOL_SLOTS=$(PSP_BATCH_POOL_SLOTS)
CFLAGS += -DPSP_BATCH_POOL_VERTICES=$(PSP_BATCH_POOL_VERTICES)
endif
ifeq ($(USE_LOCAL_PSPGL),1)
...
$(PROFILE_METADATA): $(PSP_EBOOT) $(PSP_ELF) $(PSP_MAP) Makefile
	...
	$(V){ \
		...
		printf 'PSP_MERGE_ANALYSIS=%s\n' '$(PSP_MERGE_ANALYSIS)'; \
		printf 'PSP_BATCH_POOL=%s slots=%s verts=%s\n' '$(PSP_BATCH_POOL)' '$(PSP_BATCH_POOL_SLOTS)' '$(PSP_BATCH_POOL_VERTICES)'; \
