...
VTX_FUSED_TNL ?= 1
PSP_FLOAT_MTX ?= 1
PSPGL_SWAP_INTERVAL ?= 0
PSP_MAP_SKIP_COMPLETED_PATHS ?= 0
PSP_MAP_SKIP_HISTORY_HUD ?= 0
PSP_MAP_SKIP_PLANETS ?= 0
PSP_NATIVE_MAP_PLANETS ?= 0
PSP_NATIVE_MAP_METEO ?= 0
USE_LOCAL_PSPGL ?= 1
PSP_LOG ?= 0
PSP_TRACE ?= 0
...
ifneq ($(PSP_FLOAT_MTX),0)
ifneq ($(PSP_FLOAT_MTX),1)
$(error PSP_FLOAT_MTX must be 0 or 1)
endif
endif
...
ifneq ($(filter 0 1,$(PSP_MAP_SKIP_COMPLETED_PATHS)),$(PSP_MAP_SKIP_COMPLETED_PATHS))
$(error PSP_MAP_SKIP_COMPLETED_PATHS must be 0 or 1)
endif
ifneq ($(filter 0 1,$(PSP_MAP_SKIP_HISTORY_HUD)),$(PSP_MAP_SKIP_HISTORY_HUD))
$(error PSP_MAP_SKIP_HISTORY_HUD must be 0 or 1)
endif
ifneq ($(filter 0 1,$(PSP_MAP_SKIP_PLANETS)),$(PSP_MAP_SKIP_PLANETS))
$(error PSP_MAP_SKIP_PLANETS must be 0 or 1)
endif
ifneq ($(filter 0 1,$(PSP_NATIVE_MAP_PLANETS)),$(PSP_NATIVE_MAP_PLANETS))
$(error PSP_NATIVE_MAP_PLANETS must be 0 or 1)
endif
ifneq ($(filter 0 1,$(PSP_NATIVE_MAP_METEO)),$(PSP_NATIVE_MAP_METEO))
$(error PSP_NATIVE_MAP_METEO must be 0 or 1)
endif
...
CFLAGS += -DPSPGL_SWAP_INTERVAL=$(if $(filter 1,$(PSPGL_SWAP_INTERVAL)),1,0)
CFLAGS += -DPSP_MAP_SKIP_COMPLETED_PATHS=$(if $(filter 1,$(PSP_MAP_SKIP_COMPLETED_PATHS)),1,0)
CFLAGS += -DPSP_MAP_SKIP_HISTORY_HUD=$(if $(filter 1,$(PSP_MAP_SKIP_HISTORY_HUD)),1,0)
CFLAGS += -DPSP_MAP_SKIP_PLANETS=$(if $(filter 1,$(PSP_MAP_SKIP_PLANETS)),1,0)
CFLAGS += -DPSP_NATIVE_MAP_PLANETS=$(if $(filter 1,$(PSP_NATIVE_MAP_PLANETS)),1,0)
CFLAGS += -DPSP_NATIVE_MAP_METEO=$(if $(filter 1,$(PSP_NATIVE_MAP_METEO)),1,0)
...
psp-profile-hw-counters:
	$(MAKE) PROFILE_PSP=0 PROFILE_PHASES=0 PROFILE_COMPONENTS=0 PROFILE_HW_COUNTERS=1 PROFILE_HW_COUNTER_SCOPES=$(PROFILE_HW_COUNTER_SCOPES) PROFILE_HW_COUNTER_FRAMES=$(PROFILE_HW_COUNTER_FRAMES) PROFILE_HW_COUNTER_WARMUP_FRAMES=$(PROFILE_HW_COUNTER_WARMUP_FRAMES) BUILD_DIR=build/psp-profile-hw-counters PSP_FPS_OVERLAY=$(PSP_FPS_OVERLAY) PSP_LOG=$(PSP_LOG) PSP_TRACE=$(PSP_TRACE) PSP_RENDERER_DIAGNOSTICS=$(PSP_RENDERER_DIAGNOSTICS) PSP_AUDIO=$(PSP_AUDIO) PSPGL_SWAP_INTERVAL=$(PSPGL_SWAP_INTERVAL) PSP_MAP_SKIP_COMPLETED_PATHS=$(PSP_MAP_SKIP_COMPLETED_PATHS) PSP_MAP_SKIP_HISTORY_HUD=$(PSP_MAP_SKIP_HISTORY_HUD) PSP_MAP_SKIP_PLANETS=$(PSP_MAP_SKIP_PLANETS) PSP_NATIVE_MAP_PLANETS=$(PSP_NATIVE_MAP_PLANETS) PSP_NATIVE_MAP_METEO=$(PSP_NATIVE_MAP_METEO) USE_LOCAL_PSPGL=$(USE_LOCAL_PSPGL) psp
...
$(PROFILE_METADATA): $(PSP_EBOOT) $(PSP_ELF) $(PSP_MAP) Makefile
	...
	$(V){ \
		...
		printf 'PSP_FLOAT_MTX=%s\n' '$(PSP_FLOAT_MTX)'; \
		printf 'PSP_MAP_SKIP_COMPLETED_PATHS=%s\n' '$(PSP_MAP_SKIP_COMPLETED_PATHS)'; \
		printf 'PSP_MAP_SKIP_HISTORY_HUD=%s\n' '$(PSP_MAP_SKIP_HISTORY_HUD)'; \
		printf 'PSP_MAP_SKIP_PLANETS=%s\n' '$(PSP_MAP_SKIP_PLANETS)'; \
		printf 'PSP_NATIVE_MAP_PLANETS=%s\n' '$(PSP_NATIVE_MAP_PLANETS)'; \
		printf 'PSP_NATIVE_MAP_METEO=%s\n' '$(PSP_NATIVE_MAP_METEO)'; \
		printf 'PSPGL_SWAP_INTERVAL=%s\n' '$(PSPGL_SWAP_INTERVAL)'; \
		printf 'PSP_AUDIO=%s\n' '$(PSP_AUDIO)'; \
